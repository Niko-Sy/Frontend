  ...
  <PageContainer title="发布信息" subtitle="四步完成发布，系统会提示可能重复的信息">
    <el-card class="publish-card" shadow="never">
      ...
      <div v-if="active === 0" class="type-choice">
        <button :class="{ active: form.itemType === 'LOST' }" @click="form.itemType = 'LOST'">
          <strong>我丢失了物品</strong>
          <span>发布失物信息，等待他人提供线索。</span>
        </button>
        <button :class="{ active: form.itemType === 'FOUND' }" @click="form.itemType = 'FOUND'">
          <strong>我拾到了物品</strong>
          <span>发布拾物信息，等待失主认领。</span>
        </button>
      </div>
